...
def chunk_consumer():
    global audio_buffer
    print(f"[INFO] Target chunk size: {CHUNK_SAMPLES} samples (~{POSE_LEN / POSE_FPS:.2f}s)")
    ...
    while not stop_event.is_set():
        ...
        if chunk is not None:
            ...
            header_lines[file_content_length - 2] = f"Frames: {rec_pose.shape[0] - 1}\n"
            ...
            print(f"Inference time chunk {chunk_id}: {time.time()-t}\n")
            chunk_id += 1
        else:
            time.sleep(0.01)
...
def main():
    try:
        print("[INFO] Starting audio input stream...")
        ...
        print("[INFO] Microphone is recording... Press Ctrl+C to stop.")
        while not stop_event.is_set():
            time.sleep(0.1)
        ...
        try:
            ...
            print(f"[WARN] Error closing stream: {e}")
        time.sleep(0.5)

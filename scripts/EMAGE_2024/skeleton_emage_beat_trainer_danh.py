...
class CustomTrainer(train.BaseTrainer):
    def __init__(self, args, loader_type="train", build_cache=False): # adding build_cache=True
        ...
        self.args.vae_test_dim = 36
        self.vq_model_upper = getattr(vq_model_module, "VQVAEConvZero")(self.args).to(self.rank)
        other_tools.load_checkpoints(self.vq_model_upper, "./outputs/audio2pose/custom/0701_111118_cnn_vqvae_upper_30/last_799.bin", args.e_name)
        ...
        other_tools.load_checkpoints(self.vq_model_face, "./outputs/audio2pose/custom/0701_142801_cnn_vqvae_face_30_skeleton/last_799.bin", args.e_name)
        ...
        checkpoint = other_tools.load_checkpoints(self.vq_model_hands, "./outputs/audio2pose/custom/0701_111118_cnn_vqvae_hand_30/last_799.bin", args.e_name)
        ...
        other_tools.load_checkpoints(self.vq_model_lower, "./outputs/audio2pose/custom/0701_114406_cnn_vqvae_lower_30/last_799.bin", args.e_name)
        ...
        other_tools.load_checkpoints(self.global_motion, "./outputs/audio2pose/custom/0701_114406_cnn_vqvae_lower_30/last_799.bin", args.e_name)
    ...
    def inverse_selection_tensor(self, filtered_t, selection_array, n):
        ...
        original_shape_t = torch.zeros((n, 225)).cuda()
    ...
    def _load_data(self, dict_data):
        ...
        tar_pose_6d = rc.axis_angle_to_matrix(tar_pose.reshape(bs, n, 75, 3))
    ...
    def _g_training(self, loaded_data, use_adv, mode="train", epoch=0):
        ...
        if mode != 'train':
            ...
            # print(f"rec_upper: {rec_upper.shape}")
            rec_pose_upper = rec_upper.reshape(bs * n, 12 * 3)
            ...
            rec_pose_lower = rec_lower.reshape(bs * n, 12 * 3)
            ...
            rec_pose_hands = rec_hands.reshape(bs * n, 48 * 3)
            ...
            rec_pose = rec_pose.reshape(bs, n, 75*3)
    ...
    def _g_test(self, loaded_data):
        ...
        for i in range(0, roundt):
            ...
            rec_pose_upper = rec_upper_last.reshape(bs * n, 12 * 3)
            rec_pose_upper_recover = self.inverse_selection_tensor(rec_pose_upper, self.joint_mask_upper, bs*n)
            ...
            rec_pose_lower = rec_lower_last.reshape(bs * n, 12 * 3)
            rec_pose_lower_recover = self.inverse_selection_tensor(rec_pose_lower, self.joint_mask_lower, bs*n)
            ...
            rec_pose_hands = rec_hands_last.reshape(bs * n, 48 * 3)
            rec_pose_hands_recover = self.inverse_selection_tensor(rec_pose_hands, self.joint_mask_hands, bs*n)
    ...
    def test(self, epoch):
        ...
        with torch.no_grad():
            for its, batch_data in enumerate(self.test_loader):
                ...
                out_sub_face = rec_exps.cpu().numpy().reshape(-1, self.args.face_dims)

                out_final = out_sub
                out_final_face = out_sub_face
                if i != 0:
                    out_final = np.concatenate((out_final,out_sub), 0)
                    out_final_face = np.concatenate((out_final,out_sub), 0)
                else:
                    out_final = out_sub
                    out_final_face = out_final_face

                print(f"out_final (pose) shape: {out_final.shape}")
                print(f"out_final_face shape: {out_final_face.shape}")
    ...
    #             print(f"rec_pose, tar_pose: {rec_pose.shape}, {tar_pose.shape}") # torch.Size([1, 2044, 

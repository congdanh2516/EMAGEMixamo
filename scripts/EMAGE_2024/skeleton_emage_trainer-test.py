...
class CustomTrainer(train.BaseTrainer):
    def __init__(self, args, loader_type="train", build_cache=False): # adding build_cache=True
        ...
        other_tools.load_checkpoints(self.vq_model_upper, "./outputs/audio2pose/custom/200_upper_182923032025/last_199.bin", args.e_name)
        ...
        other_tools.load_checkpoints(self.vq_model_lower, "./outputs/audio2pose/custom/0306_215727_cnn_vqvae_lower_30/last_1.bin", args.e_name)
    ...
    def _g_training(self, loaded_data, use_adv, mode="train", epoch=0):
        ...
        if mode != 'train':
            ...
            # print(f"rec_upper: {rec_upper.shape}")
            rec_pose_upper = rec_upper.reshape(bs * n, 13 * 3)
            ...
            rec_pose_lower = rec_lower.reshape(bs * n, 11 * 3)
    ...
    def _g_test(self, loaded_data):
        ...
        for i in range(0, roundt):
            ...
            rec_pose_upper = rec_upper_last.reshape(bs * n, 13 * 3)
            rec_pose_upper_recover = self.inverse_selection_tensor(rec_pose_upper, self.joint_mask_upper, bs*n)
            ...
            rec_pose_lower = rec_lower_last.reshape(bs * n, 11 * 3)
            rec_pose_lower_recover = self.inverse_selection_tensor(rec_pose_lower, self.joint_mask_lower, bs*n)
...
#                     continue
    ...
    def test(self, epoch):
        ...
        total_length = 0
        # print(f"Selected test files: {self.test_data}")
        # test_seq_list = self.test_data.selected_file
        test_seq_list = os.listdir(self.test_demo)
        print(f"test_seq_list: {test_seq_list}")

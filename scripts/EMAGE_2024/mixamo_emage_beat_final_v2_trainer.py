...
class CustomTrainer(train.BaseTrainer):
    def __init__(self, args):
        ...
        # other_tools.load_checkpoints(self.vq_model_upper, "/work/vanthoai97/beat_vicon_hips/pretrained/upper/last_399.bin", args.e_name)
        ...
        other_tools.load_checkpoints(self.vq_model_face, "/data/nas07/PersonalData/thoai/beat_vicon_hips/pretrained/mixamo/face/last_671.bin", args.e_name)
        # other_tools.load_checkpoints(self.vq_model_face, "/work/vanthoai97/beat_vicon_hips/pretrained/face/last_4.bin", args.e_name)
        ...
        checkpoint = other_tools.load_checkpoints(self.vq_model_hands, "/data/nas07/PersonalData/thoai/beat_vicon_hips/pretrained/mixamo/hands/last_795.bin", args.e_name)
        # checkpoint = other_tools.load_checkpoints(self.vq_model_hands, "/work/vanthoai97/beat_vicon_hips/pretrained/hands/last_199.bin", args.e_name)
        ...
        ## Global motion
        self.args.vae_test_dim = 312 # N.o joint nhân 6
        self.args.vae_layer = 4
    ...
    def _load_data(self, dict_data):
        ...
        tar_pose = tar_pose_raw[:, :, :156].to(self.rank) # tới số joint nhân 3
        # print("tar_pose shape:", tar_pose.shape)
        tar_contact = tar_pose_raw[:, :, 156:156+2].to(self.rank) # 2 giá trị tiếp theo của pose laf contact
    ...
    def _g_training(self, loaded_data, use_adv, mode="train", epoch=0):
        ...
        mask_val = torch.ones(bs, n, self.args.pose_dims+3+2).float().cuda() ## 3 là trans, 2 là contact
        ...
        #     # ------ masked gesture moderling------ #
            ...
            mask = torch.rand(bs, n, self.args.pose_dims+3+2) < mask_ratio
        ...
        if mode != 'train':
            ...
            rec_pose_legs = rec_lower[:, :, :54] # Số joint của lower nhân 6
    ...
    def _g_test(self, loaded_data):
        ...
        for i in range(0, roundt):
            ...
            ###
            rec_pose_legs = rec_lower_last[:, :, :54] # Số joint của lower nhân 6
    ...
    def val(self, epoch):
        ...
        with torch.no_grad():
            ...
            # self.tracker.update_meter("acc", "val", acceleration_loss)
                ...
                # === Free up memory ===
                del rec_pose, tar_pose, net_out, loaded_data

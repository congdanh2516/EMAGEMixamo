...
class BaseTrainer(object):
    def __init__(self, args, sp, ap, tp):
        ...
        if _global_inference == False:
            ...
            other_tools.load_checkpoints(_vq_model_face, "/data/nas07/PersonalData/danh/source/mixamo_v2.1/pretrained/mixamo/face/last_699.bin", args.e_name)
    
            # upper model
            self.args.vae_test_dim = 66
            _vq_model_upper = getattr(vq_model_module, "VQVAEConvZero")(self.args).to(self.rank)
            other_tools.load_checkpoints(_vq_model_upper, "/data/nas07/PersonalData/danh/source/mixamo_v2.1/pretrained/mixamo/upper/last_799.bin", args.e_name)
    
            # hand model
            self.args.vae_test_dim = 192
            _vq_model_hands = getattr(vq_model_module, "VQVAEConvZero")(self.args).to(self.rank)
            other_tools.load_checkpoints(_vq_model_hands, "/data/nas07/PersonalData/danh/source/mixamo_v2.1/pretrained/mixamo/hands/last_795.bin", args.e_name)
    
            # lower model
            self.args.vae_test_dim = 59
            self.args.vae_layer = 4
            _vq_model_lower = getattr(vq_model_module, "VQVAEConvZero")(self.args).to(self.rank)
            other_tools.load_checkpoints(_vq_model_lower, "/data/nas07/PersonalData/danh/source/mixamo_v2.1/pretrained/mixamo/lowerfoot/last_486.bin", args.e_name)
    
            # global motion
            self.args.vae_test_dim = 59
            self.args.vae_layer = 4
            _global_motion = getattr(vq_model_module, "VAEConvZero")(self.args).to(self.rank)
            other_tools.load_checkpoints(_global_motion, "/data/nas07/PersonalData/danh/source/mixamo_v2.1/pretrained/mixamo/lowerfoot/last_486.bin", args.e_name)

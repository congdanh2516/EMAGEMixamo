...
class CustomTrainer(train.BaseTrainer):
    ...
    def __init__(self, args):
        ...
        self.loss_meters = {
            'val_loss_all': other_tools.AverageMeter('val_loss_all'),
            'foot_val': other_tools.AverageMeter('foot_val'),
            'contact_val': other_tools.AverageMeter('contact_val'),
            'trans_vel_val': other_tools.AverageMeter('trans_vel_val'),
            'trans_val': other_tools.AverageMeter('trans_val'),
            
            'loss_all': other_tools.AverageMeter('loss_all'),
            'foot_loss': other_tools.AverageMeter('foot_loss'),
            'contact_loss': other_tools.AverageMeter('contact_loss'),
            'trans_vel_loss': other_tools.AverageMeter('trans_vel_loss'),
            'trans_loss': other_tools.AverageMeter('trans_loss'),
               
        }
        self.best_epochs = {
            'val_loss_all': [np.inf, 0],
            'foot_val': [np.inf, 0],
            'contact_val': [np.inf, 0],
            'trans_vel_val': [np.inf, 0],
            'trans_val': [np.inf, 0],
                            }
    ...
    def train(self, epoch):
        ...
        for its, dict_data in enumerate(self.train_loader):
            ...
            tar_trans_vel_x = other_tools.estimate_linear_velocity(tar_trans[:, :, 1:2], dt=1/self.args.pose_fps)
            ...
            ##-----------------Loss for contact------------##
            rec_contact = net_out["rec_pose"][:, :, j*6+3:j*6+3+2] # j*6 là vị trí kết thúc của pose, +3 của trans
            ...
            rec_x_trans = other_tools.velocity2position(rec_trans[:, :, 1:2], 1/self.args.pose_fps, tar_trans[:, 0, 0:1])
            rec_z_trans = other_tools.velocity2position(rec_trans[:, :, 2:3], 1/self.args.pose_fps, tar_trans[:, 0, 2:3])
            rec_y_trans = rec_trans[:,:,0:1]
            rec_xyz_trans = torch.cat([rec_x_trans, rec_y_trans, rec_z_trans], dim=-1)
            
            loss_trans_vel = self.vel_loss(rec_trans[:, :, 1:2], tar_trans_vel_x) * self.args.rec_weight \
            + self.vel_loss(rec_trans[:, :, 2:3], tar_trans_vel_z) * self.args.rec_weight 
            v3 =  self.vel_loss(rec_trans[:, :, 1:2][:, 1:] - rec_trans[:, :, 1:2][:, :-1], tar_trans_vel_x[:, 1:] - tar_trans_vel_x[:, :-1]) * self.args.rec_weight \
            ...
            self.loss_meters['trans_vel_loss'].update(loss_trans_vel.item()) 
            loss += loss_trans_vel

            loss_trans = self.vel_loss(rec_xyz_trans, tar_trans) * self.args.rec_weight 
            self.loss_meters['trans_loss'].update(loss_trans.item())
            loss += loss_trans
            ...
            foot_right = net_out["rec_pose"][:, :, 4*6:4*6+6].reshape(bs, n, 1, 6)   # joints 4 to 6 (3 joints * 6 rotation6d)
            # print("foot_right", foot_right.shape)
            foot_left = net_out["rec_pose"][:, :, 8*6:8*6+6].reshape(bs, n, 1, 6)  # joints 10 to 12 (3 joints * 6 rotation6d)
            ...
            self.loss_meters['loss_all'].update(loss.item())
    ...
    def val(self, epoch):
        ...
        with torch.no_grad():
            ...
            for its, dict_data in enumerate(self.val_loader):
                ...
                ##--------------Loss for Position----------##
                rec_trans = net_out["rec_pose"][:, :, j*6:j*6+3]
                rec_x_trans = other_tools.velocity2position(rec_trans[:, :, 1:2], 1/self.args.pose_fps, tar_trans[:, 0, 1:2])
                rec_z_trans = other_tools.velocity2position(rec_trans[:, :, 2:3], 1/self.args.pose_fps, tar_trans[:, 0, 2:3])
                rec_y_trans = rec_trans[:,:,0:1]
                rec_xyz_trans = torch.cat([rec_x_trans, rec_y_trans, rec_z_trans], dim=-1)
                ...
                v3 =  self.vel_loss(rec_trans[:, :, 1:2][:, 1:] - rec_trans[:, :, 1:2][:, :-1], tar_trans_vel_x[:, 1:] - tar_trans_vel_x[:, :-1]) * self.args.rec_weight \
                + self.vel_loss(rec_trans[:, :, 2:3][:, 1:] - rec_trans[:, :, 2:3][:, :-1], tar_trans_vel_z[:, 1:] - tar_trans_vel_z[:, :-1]) * self.args.rec_weight
                a3 = self.vel_loss(rec_trans[:, :, 1:2][:, 2:] + rec_trans[:, :, 1:2][:, :-2] - 2 * rec_trans[:, :, 1:2][:, 1:-1], tar_trans_vel_x[:, 2:] + tar_trans_vel_x[:, :-2] - 2 * tar_trans_vel_x[:, 1:-1]) * self.args.rec_weight \
                + self.vel_loss(rec_trans[:, :, 2:3][:, 2:] + rec_trans[:, :, 2:3][:, :-2] - 2 * rec_trans[:, :, 2:3][:, 1:-1], tar_trans_vel_z[:, 2:] + tar_trans_vel_z[:, :-2] - 2 * tar_trans_vel_z[:, 1:-1]) * self.args.rec_weight
                ...
                self.loss_meters['trans_vel_val'].update(loss_trans_vel.item()) 
                val_loss += loss_trans_vel
    
                loss_trans = self.vel_loss(rec_xyz_trans, tar_trans) * self.args.rec_weight 
                self.loss_meters['trans_val'].update(loss_trans.item())
                val_loss += loss_trans
                ...
                foot_right = net_out["rec_pose"][:, :, 4*6:6*4+6].reshape(bs, n, 1, 6)  
                # print("foot_right", foot_right.shape)
                foot_left = net_out["rec_pose"][:, :, 8*6:8*6+6].reshape(bs, n, 1, 6)  
                # print("foot_left", foot_left.shape)
                ...
                self.loss_meters['val_loss_all'].update(val_loss.item())            
        ...
        with torch.no_grad():
            ...
            for its, dict_data in enumerate(self.test_loader):
                ...
                if True:
                    ...
                    # ##-------Comment nếu muốn dùng giá trị raw từ mô hình----------##

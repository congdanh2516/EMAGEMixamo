...
def main_worker(rank, world_size, args):
    ...
    if args.resume_path is not None and os.path.isfile(args.resume_path):
        ...
        start_epoch = other_tools.load_checkpoints_resume(
            model=trainer.model,
            opt=trainer.opt,
            scheduler=trainer.opt_s,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if trainer.rank == 0:
            other_tools.save_checkpoints(
                ...
                trainer.model,  # `save_checkpoints` will unwrap DDP internally
                opt=trainer.opt,
                epoch=epoch,
                lrs=trainer.opt_s
            )
    ...
    # if rank == 0:
    #     wandb.finish()
    for k, (best_value, best_epoch) in trainer.best_epochs.items():
        if trainer.writer is not None:
            trainer.writer.add_scalar(f"{k}_best", best_value, best_epoch)
            trainer.writer.add_scalar(f"{k}_epoch", best_epoch, 0)
            print(f"{k}_best", best_value, best_epoch)
...
def make_args_picklable(args):
    ...
    for k, v in vars(args).items():
        try:
            ...
            print(f"Skipping non-picklable arg '{k}': {e}")
    return Namespace(**clean_dict)
...
if __name__ == "__main__":
    ...
    os.environ["MASTER_PORT"] = os.environ.get("MASTER_PORT", '8684')
    os.environ["TORCH_DISTRIBUTED_DEBUG"] = "DETAIL"
    args = config.parse_args()
    
    # raw_args = config.parse_args()

    # # ✅ Safely convert raw_args to a plain dict
    # if isinstance(raw_args, argparse.Namespace):
    #     args_dict = vars(raw_args)
    # elif OmegaConf.is_config(raw_args):  # OmegaConf DictConfig
    #     args_dict = OmegaConf.to_container(raw_args, resolve=True)
    # else:
    #     raise TypeError(f"Unsupported args type: {type(raw_args)}")

    # # ✅ Convert to SimpleNamespace so you can use dot-access AND make it pickleable
    ...
    #     pickle.dumps(args)
    # except Exception as e:
    #     print("Pickling args failed due to:", e)
    #     for k, v in vars(args).items():
    #         try:
    #             pickle.dumps(v)
    #         except Exception as inner_e:
    #             print(f"Cannot pickle args.{k} = {v} --> {inner_e}")
    # args = make_args_picklable(args)  # Clean args first
    ...
    if args.ddp:
        ...
        # Use mp.spawn to launch multiple processes
        mp.spawn(main_worker, args=( world_size, args), nprocs=world_size, join=True)
    else:
        main_worker(0, len(args.gpus), args)
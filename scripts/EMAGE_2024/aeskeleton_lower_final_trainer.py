...
class CustomTrainer(train.BaseTrainer):
    ...
    def train(self, epoch):
        ...
        for its, dict_data in enumerate(self.train_loader):
            ...
            tar_pose = tar_pose_raw[:, :, :39].cuda() 
            tar_contact = tar_pose_raw[:, :, 39:45].cuda() 
            ...
            ##-----------------Loss for contact------------##
            rec_contact = net_out["rec_pose"][:, :, j*6+3:j*6+9] # j*6 là vị trí kết thúc của pose, +3 của trans
            ...
            ##--------------Loss for foot-----------##
            model_contact = net_out["rec_pose"][:, :, j*6+3:j*6+9] 
            ...
            foot_right = net_out["rec_pose"][:, :, 4*6:6*6+6].reshape(bs, n, 3, 6)   # joints 4 to 6 (3 joints * 6 rotation6d)
            # print("foot_right", foot_right.shape)
            foot_left = net_out["rec_pose"][:, :, 10*6:12*6+6].reshape(bs, n, 3, 6)  # joints 10 to 12 (3 joints * 6 rotation6d)
    ...
    def val(self, epoch):
        ...
        with torch.no_grad():
            ...
            for its, dict_data in enumerate(self.val_loader):
                ...
                ##-----------------Loss for contact------------##
                rec_contact = net_out["rec_pose"][:, :, j*6+3:j*6+9]
                loss_contact = self.vectices_loss(rec_contact, tar_contact) * self.args.rec_weight * self.args.rec_pos_weight

...
class BaseTrainer(object):
    def __init__(self, args):
        ...
        self.checkpoint_path = args.out_path + "custom/" + args.name + args.notes + "/" #wandb.run.dir #args.cache_path+args.out_path+"/"+args.name # xem lại path này
        if self.rank == 0:
        ...
        other_tools.load_checkpoints(self.vq_model_face, "/home/caocongdanh/Lab projects/mixamo_v2/pretrained/mixamo/face/last_699.bin", args.e_name)

        # upper model
        self.args.vae_test_dim = 66
        self.vq_model_upper = getattr(vq_model_module, "VQVAEConvZero")(self.args).to(self.rank)
        other_tools.load_checkpoints(self.vq_model_upper, "/home/caocongdanh/Lab projects/mixamo_v2/pretrained/mixamo/upper/last_799.bin", args.e_name)

        # hand model
        self.args.vae_test_dim = 192
        self.vq_model_hands = getattr(vq_model_module, "VQVAEConvZero")(self.args).to(self.rank)
        other_tools.load_checkpoints(self.vq_model_hands, "/home/caocongdanh/Lab projects/mixamo_v2/pretrained/mixamo/hands/last_795.bin", args.e_name)
        # /data/nas07/PersonalData/thoai/beat_vicon_hips/pretrained/mixamo/hands/last_795.bin

        # lower model
        self.args.vae_test_dim = 59
        self.args.vae_layer = 4
        self.vq_model_lower = getattr(vq_model_module, "VQVAEConvZero")(self.args).to(self.rank)
        other_tools.load_checkpoints(self.vq_model_lower, "/home/caocongdanh/Lab projects/mixamo_v2/pretrained/mixamo/lowerfoot/last_486.bin", args.e_name)

        # global motion
        self.args.vae_test_dim = 59
        self.args.vae_layer = 4
        self.global_motion = getattr(vq_model_module, "VAEConvZero")(self.args).to(self.rank)
        other_tools.load_checkpoints(self.global_motion, "/home/caocongdanh/Lab projects/mixamo_v2/pretrained/mixamo/lowerfoot/last_486.bin", args.e_name)
    ...
    def _load_data(self, dict_data):
        ...
        tar_index_value_face_top = self.vq_model_face.map2index(tar_pose_face) # bs*n/4 # tar_pose_face.shape [1, 1]
    ...
    def test_demo(self, epoch):
        ...
        # test_demo: /data/nas07/PersonalData/danh/EMAGEMixamo/mixamo_english_30_4_811_all/test_norm/
        test_demo = self.args.root_path + self.args.test_data_path + "bvh_full/"
        # print(f"test_demo: {test_demo}")
        # test_demo = self.args.
        ...
        with torch.no_grad():
            for its, batch_data in enumerate(self.test_loader):
                ...
                zero_trans = np.zeros((bs * n, 3))
                rec_pose = np.concatenate([zero_trans, rec_pose], axis=1) # (frames*1, joint*3+3)
                ...
                wavfile.write(f"{results_save_path}in_audio_{seq_name}.wav", sample_rate, audio_int16)

...
class CustomDataset(Dataset):
    def __init__(self, args, loader_type, smplx_path=None, audio_path=None, text_path=None, augmentation=None, kwargs=None, build_cache=True):
        ...
        # self.smplx_path = "/data/nas07/PersonalData/danh/EMAGEMixamo/mixamo_english_30_4_811_all/test_norm/bvh_full/1_fufu_0_7_7.bvh"
        self.smplx_path = self.args.code_path + "/scripts/EMAGE_2024/utils/assets/1_fufu_0_7_7_full.bvh"
        self.audio_path = audio_path
        self.text_path = None # "./EMAGE/test_sequences/textgrid/2_scott_0_1_1.TextGrid"
        ...
        # print(f"self.selected_file: {self.selected_file}")
        # self.selected_file = split_rule
        ...
        # preloaded_dir = "/data/nas07/PersonalData/danh/EMAGEMixamo/mixamo_english_30_4_811_all/test_norm/all_pose2/bvh_rot_full_cache"
        preloaded_dir = self.args.code_path + "/datasets/mixamo_cache"
    ...
    def cache_generation(self, out_lmdb_dir, disable_filtering, clean_first_seconds,  clean_final_seconds, is_test=False):      
        ...
        if "smplx" in self.args.pose_rep:
            ...
            ## Detect contact for foot 4, 8 (based 0)
            foot_joints_part1 = pose_each_file[:, 12:15]    #(joints * 3 coords)
            # print("foot_joints_part1", foot_joints_part1.shape)
            foot_joints_part2 = pose_each_file[:, 24:27]    #(joints * 3 coords)
            ...
            foot_joints = foot_joints.reshape(rot_data.shape[0], 2, 3)
            ...
            # facial_file = pose_file.replace("bvh_full", "facial52").replace("bvh", "json")
            ...
            if self.args.facial_rep is not None:
                ...
                # facial_file = pose_file.replace("bvh_full", "facial52").replace("bvh", "json")
                # facial_file = "/data/nas07/PersonalData/danh/EMAGEMixamo/mixamo_english_30_4_811_all/test_norm/facial52/1_fufu_0_7_7.json"
                facial_file = self.args.code_path + "/scripts/EMAGE_2024/utils/assets/1_fufu_0_7_7.json"
                # print(f"facial_file: {facial_file}")
                try:
                    with open(facial_file, 'r') as facial_data_file:
                        facial_data = json.load(facial_data_file)
                        for j, frame_data in enumerate(facial_data['frames']):
                    ...
                    logger.warning(f"{e}# ---- file not found for Facial {id_pose}, skip all files with the same id ---- #")
    ...
    def _sample_from_clip(
        ...
        # if audio_each_file != []:  # phiên bản emagepy38
        if audio_each_file.size>0:
        ...
        for ratio in self.args.multi_length_training:
            ...
            for i in range(num_subdivision): # cut into around 2s chip, (self npose)
                ...
                if sample_pose.any() != None:
                    ...
                    # is_correct_motion = (sample_pose != [])  # phiên bản emagepy38
                    is_correct_motion = (sample_pose.size>0)
            ...
            if len(sample_pose_list) > 0:
                with dst_lmdb_env.begin(write=True) as txn:
                    ...
                    for pose, audio, facial, shape, trans, vid in zip(
                        sample_pose_list,
                        sample_audio_list,
                        sample_facial_list,
                        sample_shape_list,
                        ...
                        v = [pose, audio, facial, shape, trans, vid]
    ...
    def __getitem__(self, idx):
        with self.lmdb_env.begin(write=False) as txn:
            ...
            tar_pose, in_audio, in_facial, in_shape, trans, vid = sample
            #print(in_shape)
            ...
            if self.loader_type == "test":
                ...
                # print(f"1in_facial {in_facial}")
                if isinstance(in_facial, list):
                    in_facial = np.array(in_facial)
                # print(f"2in_facial {in_facial}")
                # line700
            ...
            return {"pose":tar_pose, "audio":in_audio, "facial":in_facial, "beta": in_shape, "trans":trans, "id":vid}

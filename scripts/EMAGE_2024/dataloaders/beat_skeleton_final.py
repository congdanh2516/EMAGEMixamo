...
class CustomDataset(Dataset):
    ...
    def cache_generation(self, out_lmdb_dir, disable_filtering, clean_first_seconds,  clean_final_seconds, is_test=False):
        ...
        for pose_file in pose_files:
            ...
            #         print("data from file:", data.shape)
            ...
            # print("pose_frames:", pose_frames.shape) 
    ...
    def _sample_from_clip(
        ...
        min_frames = min(pose_each_file.shape[0], facial_each_file.shape[0])

        pose_each_file = pose_each_file[:min_frames]
        facial_each_file = facial_each_file[:min_frames]
        trans_each_file = trans_each_file[:min_frames]
        shape_each_file = shape_each_file[:min_frames]
# Similarly for other related arrays

        round_seconds_skeleton = min_frames // self.args.pose_fps  # assume 1500 frames / 15 fps = 100 s
        ...
        for ratio in self.args.multi_length_training:
            ...
            if len(sample_pose_list) > 0:
                with dst_lmdb_env.begin(write=True) as txn:
                    ...
                    for pose, audio, facial, shape, word, trans in zip(
                        # sample_pose_list,
                        sample_facial_list,
                        sample_audio_list,
                        sample_facial_list,
                        ...
                        v = [pose, audio, facial, shape, word, trans]
    ...
    def __getitem__(self, idx):
        with self.lmdb_env.begin(write=False) as txn:
            ...
            tar_pose, in_audio, in_facial, in_shape, in_word, trans = sample
            #print(in_shape)
            ...
            return {"pose":tar_pose, "audio":in_audio, "facial":in_facial, "beta": in_shape, "word":in_word, "trans":trans}

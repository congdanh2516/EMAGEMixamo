...
class CustomDataset(Dataset):
    def __init__(self, args, loader_type, augmentation=None, kwargs=None, build_cache=True):
        ...
        logger.info(f"TAR JOINT LIST: {self.tar_joint_list} {len(self.tar_joint_list)}")
        ...
        if 'smplx' in self.args.pose_rep:
            ...
            # print(f"self.joint_mask.shape: {self.joint_mask.shape}")
            for idx, joint_name in enumerate(self.tar_joint_list):
                if joint_name == "Hips":
                    ...
                    self.joint_mask[self.ori_joint_list[joint_name][1] - self.ori_joint_list[joint_name][0]:self.ori_joint_list[joint_name][1]] = 1
                    # print(self.joint_mask)
    #----------------Copy from beat_sep_lower.py-----------##
        logger.info(f"JOINS MASK LEN: {len(self.joint_mask)}")
        logger.info(f"JOINS MASK: {self.joint_mask}")
        ...
        # logger.info(f"beat_skeleton - _sample_from_clip - self.mean_pose: {self.mean_pose}, {self.mean_pose.shape}")
        # logger.info(f"nan????: {np.any(np.isnan(self.mean_pose))}") # True


        # logger.info(f"beat_skeleton - _sample_from_clip - self.std_pose: {self.std_pose}, {self.std_pose.shape}")
        # logger.info(f"nan????: {np.any(np.isnan(self.std_pose))}") # True
    ...
    def build_cache(self, preloaded_dir):
        ...
        logger.info("Creating the dataset cache... in beat_skeleton")
        if self.new_cache:
    ...
    def cache_generation(self, out_lmdb_dir, disable_filtering, clean_first_seconds,  clean_final_seconds, is_test=False):
        ...
        for pose_file in pose_files:
            ...
            # print(f"ID_pose: {id_pose}")
            int_value = self.idmapping(int(id_pose.split("_")[0]))
            # print(f"int_value: {id_pose}, {int_value}")
            logger.info(colored(f"# ---- Building cache beat for Pose   {id_pose} ---- #", "blue"))
            ...
            with open(pose_file, "r") as pose_data:
                for j, line in enumerate(pose_data.readlines()):
                    ...
                    # logger.info(f"beat_skeleton_new - cache_generation - data: {data}, {data.shape}")
                    data =  np.array(data)
                    # logger.info(f"beat_skeleton - cache_generation - raw_data: {data}, {data.shape}")
                    # logger.info(f"DATA SHAPE 1: {data.shape}")
                    # logger.info(f"JOINT MASK: {self.joint_mask} {len(self.joint_mask)}")
                    data = data * self.joint_mask
                    # logger.info(f"beat_skeleton - cache_generation - raw_data * self.joint_mask: {data}, {data.shape}")
                    # logger.info(f"DATA SHAPE 2: {data.shape}")
                    data = data[self.joint_mask.astype(bool)]
                    # logger.info(f"beat_skeleton - cache_generation - data[self.joint_mask.astype(bool)]: {data}, {data.shape}")
                    # logger.info(f"DATA SHAPE 3: {data.shape}")
                    pose_each_file.append(data)
                    # logger.info(f"POSE EACH FILE: {data.shape}")
            ...
            #logger.info(f"beat_skeleton - cache_generation - np.array(pose_each_file): {pose_each_file}, {pose_each_file.shape}")
            #logger.info(f"nan????: {np.any(np.isnan(pose_each_file))}") # False
            ...
            #     logger.info(f"# ---- Building cache beat for Audio  {id_pose} and Pose {id_pose} ---- #")
    ...
    def _sample_from_clip(
        ...
        # logger.info(f"beat_skeleton - _sample_from_clip - pose_each_file: {pose_each_file}, {pose_each_file.shape}")
        # logger.info(f"nan????: {np.any(np.isnan(pose_each_file))}") # False
        ...
        for ratio in self.multi_length_training:
            ...
            for i in range(num_subdivision): # cut into around 2s chip, (self npose)
                ...
                # logger.info(f"beat_skeleton - _sample_from_clip - sample_pose: {sample_pose}, {sample_pose.shape}")
                # logger.info(f"nan????: {np.any(np.isnan(sample_pose))}") # False
            ...
            # logger.info(f"beat_skeleton - _sample_from_clip - sample_pose_list: {sample_pose_list}")
            # has_nan = any(np.isnan(arr).any() for arr in sample_pose_list)
            # logger.info(f"nan????: {has_nan}") #False
            ...
            if len(sample_pose_list) > 0:
                with dst_lmdb_env.begin(write=True) as txn:
                    for pose, audio, facial, word, vid, emo, sem in zip(sample_pose_list,
                        ...
                        # logger.info(f"beat_skeleton - _sample_from_clip - normalized_pose: {normalized_pose}, {normalized_pose.shape}")
                        # logger.info(f"nan????: {np.any(np.isnan(normalized_pose))}") # True
                        

                        # logger.info(f"beat_skeleton - _sample_from_clip - pose: {pose}, {pose.shape}")
                        # logger.info(f"nan????: {np.any(np.isnan(pose))}") # True
                        ...
                        # logger.info(f"beat_skeleton - _sample_from_clip - self.joint_mask: {self.joint_mask}, {self.joint_mask.shape}")
                        # logger.info(f"nan????: {np.any(np.isnan(self.joint_mask))}") # True
